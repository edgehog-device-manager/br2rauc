...
# HACK: to make the directory writable since they are used by docker, this can be done in the
#       overlay but it is easier to do it here
mkdir -vp "${TARGET_DIR}/var/lib/opt"
rm -vr "${TARGET_DIR}/var/lib/etc-docker" || true
if [ -d "${TARGET_DIR}/etc/docker" ]; then
    mv -vf "${TARGET_DIR}/etc/docker/" "${TARGET_DIR}/var/lib/etc-docker"
fi
rm -vrf "${TARGET_DIR}/opt" || true
ln -vsf ../var/lib/opt "${TARGET_DIR}/opt"
ln -vsf ../../var/lib/etc-docker "${TARGET_DIR}/etc/docker"
